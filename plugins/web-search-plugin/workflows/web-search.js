...
const searchResult = await agent(
  ...
  { agentType: 'web-search-plugin:web-search-scout', phase: 'Search', label: 'scout', schema: SEARCH_SCHEMA },
)
if (!searchResult) {
  ...
  await parallel(
    urls.map((url) => () =>
      agent(`質問: ${query}\n\n以下のURLを取得し、この質問への回答として適切かを判定してください: ${url}`, {
        agentType: 'web-search-plugin:web-search-fetcher',
        phase: 'Fetch',
        label: 'fetch:' + url,
        schema: FETCH_SCHEMA,
      }),
...
const selection = await agent(
  ...
  { agentType: 'web-search-plugin:web-search-selector', phase: 'Select', label: 'selector', schema: SELECT_SCHEMA },
)
const rankedUrls =
...
for (const url of rankedUrls.slice(0, MAX_VERIFY_ATTEMPTS)) {
  const result = await agent(
    ...
    { agentType: 'web-search-plugin:web-search-verifier', phase: 'Verify', label: 'verify:' + url, schema: VERIFY_SCHEMA },
  )
