...
tools: mcp__plugin_web-search-plugin_fetch__fetch, mcp__plugin_web-search-plugin_playwright__browser_navigate, mcp__plugin_web-search-plugin_playwright__browser_snapshot
model: sonnet
color: yellow
---
...
## Step 1: 独立した再取得
1. `mcp__plugin_web-search-plugin_fetch__fetch`でURLを取得する (要約を挟まないMarkdown変換。既定では本文のみ抽出するため、目的の記述が見つからない・抽出結果が不自然に短い場合は`extract_main_content: false`で再取得する。既定では20万文字を超えるとエラーになるため、全文が必要な長大文書のみ`force_full: true`を指定する)
2. 以下に該当する場合のみPlaywrightへフォールバックする (目安であり厳密な閾値ではない)
   ...
   → `mcp__plugin_web-search-plugin_playwright__browser_navigate` + `mcp__plugin_web-search-plugin_playwright__browser_snapshot`で再取得する
