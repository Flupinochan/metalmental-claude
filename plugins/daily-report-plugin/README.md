...
| Command                                          | Description                                                |
| --------------------------------------------------- | -------------------------------------------------------------- |
| `/daily-report-plugin:daily-report [YYYY-MM-DD]` | Generates a JST daily work-report timeline; defaults to today |
...
sequenceDiagram
    ...
    User->>Main: /daily-report-plugin:daily-report [date]
    Main->>Main: parse $1, validate YYYY-MM-DD, default to today
    Main->>WF: Workflow({name: "daily-report-plugin:daily-report-workflow", args: {date, concurrency, minFileSize}})
...
```mermaid
flowchart TD
    A(["/daily-report-plugin:daily-report [date]"]) --> B["Discover: run fixed Python script over ~/.claude/projects/*/*.jsonl"]
